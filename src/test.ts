...
import 'mocha'
import { Connection, connect, CrdtCounter } from './antidote'
import ByteBuffer = require('bytebuffer')
import { MultidoteServer } from './multidoteServer'
...
describe('antidote client', function () {
	...
	let counterImpls = [
		...
		{
			name: 'fat-counter',
			create: (name: string) => connection.fatCounter(name),
		},
	]
	...
	describe('fat-counters', () => {
		it('should count', async () => {
			...
			await connection.update(counter.increment(3))
			let val = await counter.read()
			assert.equal(val, 3)
		})
	})
	...
	describe('flags', () => {
		it('can be set', async () => {
			...
			await connection.update(flag.set(true))
			assert.equal(await flag.read(), true)
			await connection.update(flag.set(false))
			assert.equal(await flag.read(), false)
		})
	})
	...
	describe('last-writer-wins register', () => {
		it('can be used to store and read values', async () => {
			...
			await connection.update(reg.set(['a', 'b']))
	...
	describe('multi-value register', async () => {
		it('can be used to store and read values', async () => {
			...
			await connection.update(reg.set(15))
			let val = await reg.read()
	...
	for (let setType of setTypes) {
		describe(`${setType.name}-sets`, () => {
			it('can be used to add elements', async () => {
				...
				await connection.update([set.add('x'), set.addAll(['y', [1, 2, 3]])])
			...
			it('should work with add and remove', async () => {
				...
				await connection.update(set.addAll(['a', 'b', 'c', 'd', 'e']))
				await connection.update([set.remove('a'), set.removeAll(['b', 'c'])])
				let val = await set.read()
	...
	describe('remove-resets map', () => {
		...
		it('should be possible to store and then remove things', async () => {
			...
			await connection.update([
				...
				map.removeAll([
					map.multiValueRegister('b'),
					map.multiValueRegister('c'),
				]),
			])
			let val = await map.read()
			let obj = val.toJsObject()
	...
	describe('transactions', () => {
		...
		it('can do batch-reads', async () => {
			...
			await connection.update([a.increment(1), b.increment(2), c.increment(3)])
		...
		it('can do big batch-reads', async () => {
			...
			await connection.update(registers.map(r => r.set(longStr)))
		...
		it('can do batch-reads object api', async () => {
			...
			await connection.update([objA.increment(1), objB.set('hi')])
	...
	describe('tutorial example', () => {
		it('tutorial example', async () => {
			...
			}
			{
				...
				await connection.update([set.remove('Java'), set.add('Kotlin')])
			}
