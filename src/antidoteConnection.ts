...
export class AntidoteConnection {
	...
	private readMessagesFromBuffer() {
		...
		while (buffer.remaining() >= 4) {
			...
			if (code != 0) {
				...
				let errorProto: AntidotePB.ApbErrorRespMessage = MessageCodes.antidotePb.ApbErrorResp.decode(
					slice.toBuffer(true)
				)
	...
	private handleResponse(code: number, decoded: any) {
		...
		if (request) {
			...
			console.log(
				`Unexpected response with code ${code} and value ${JSON.stringify(
					decoded
				)}`
			)
		}
	...
	private handleError(errorCode: number, errorMsg: string) {
		...
		if (request) {
			this.reject(
				request,
				new Error(`Antidote-PB Error code ${errorCode}:\n${errorMsg}`)
			)
		} else {
			console.log(
				`Unexpected error response with code ${errorCode} and message:\n ${errorMsg}`
			)
		}
	}
